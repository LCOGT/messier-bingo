...
//		this.texture = this.box.image('images/texture2.jpg',0,0,this.width,this.height);
		...
		this.nextbutton.data('mb',this).click(function(e){ this.data('mb').next(); }).mouseover(function(){ this.data('mb').nextbutton[4].attr({'fill':'#2a2521'}); }).mouseout(function(){ this.data('mb').nextbutton[4].attr({'fill':'#534741'}); });

		this.resetbutton = this.box.set();
		this.resetbutton.push(
			this.nextbutton[0].clone(), this.nextbutton[1].clone(), this.nextbutton[2].clone(), this.nextbutton[3].clone(),
			this.resetbutton[4] = this.box.path('M'+ox+','+oy+'m '+(-r*0.2)+','+(r*0.4)+' c '+(r*0.6)+',0 '+(r*0.4)+','+(-r*0.4)+' '+(r*0.1)+','+(-r*0.3)+' l 0,'+(r*0.2)+' '+(-r*0.4)+','+(-r*0.4)+' '+(r*0.4)+','+(-r*0.4)+' 0,'+(r*0.2)+' c '+(r*0.1)+',0 '+(r*0.5)+',0 '+(r*0.5)+','+(r*0.4)+' 0,'+(r*0.4)+' '+(-r*0.5)+','+(r*0.4)+' '+(-r*0.5)+','+(r*0.3)+' z').attr({'fill':'#534741','stroke':0,'cursor':'pointer','opacity':1})
		);
		this.resetbutton.data('mb',this).click(function(e){ this.data('mb').reset(); }).mouseover(function(){ this.data('mb').resetbutton[4].attr({'fill':'#2a2521'}); }).mouseout(function(){ this.data('mb').resetbutton[4].attr({'fill':'#534741'}); }).hide();
