...
(function ($) {
	...
	MessierBingo.prototype.drawBox = function(){
		...
		this.dialbg.push(
			this.box.path(makeSector(ox+3,oy,270,52,70)).attr({'fill':this.colours.deepshadow,'stroke':0}),
			...
			this.box.path(makeSector(ox+3,oy,90,47,68)).attr({'fill':this.colours.deepshadow,'stroke':0}),
		...
		this.box.transformer(this.dialbg[0],['t',1,2]);
		this.box.transformer(this.dialbg[3],['t',1,2]);
		...
		this.frame[0].attr({'fill':this.colours.deepshadow,'opacity':0.8,'stroke':0});
		this.frame[1].attr({'fill':this.colours.frame,'stroke':0});
		this.box.transformer(this.frame[0],['t',1.5,1.25])
		...
		this.overlay.push(
			this.box.path(path).attr({'fill':this.colours.shadow,'stroke':0}),
			this.box.path(path).attr({'fill':this.colours.white,'stroke':0})
		);
		this.drawText($('h1').parent(),660,100,$('h1').html().toUpperCase(),64);
		...
		this.box.transformer(this.hands[0],['R',0,910,222]);
		this.box.transformer(this.hands[1],['R',0,910,222]);
		...
		this.dialhandle.push(
			this.box.path("M "+(ox+0.5)+" "+(oy-6+0.5)+" l 5 10 t -5 30 l 0 0 t -5 -30 z").attr({'fill':this.colours.deepshadow,'stroke':0,'cursor':'pointer'}),
			this.box.path("M "+ox+" "+(oy-6)+" l 5 10 t -5 27 l 0 0 t -5 -27 z").attr({'fill':'0-'+this.colours.deepshadow+'-'+this.colours.brass+'-'+this.colours.deepshadow,'stroke':0,'cursor':'pointer'})
		);
		this.box.transformer(this.dialhandle[0],['R',0,ox,oy])
		this.box.transformer(this.dialhandle[1],['R',0,ox,oy])
	...
	function Pantograph(me,ox,oy,w,h,w2,n,p,vert){
		...
		this.group.push(
			this.box.path(this.pathoff).attr({'stroke':this.me.colours.deepshadow,'stroke-width':3}),
