/*!
	...
	(c) Stuart Lowe/LCOGT
*/
(function(){Raphael.fn.transformer=function(b,a){return b.transform(a).data("transform",a)};Raphael.fn.print_center=function(f,e,c,b,j,a){var i=this.print(f,e,c,b,j,"baseline",a);var d=i.getBBox();var h=(f-d.x)-d.width/2;var g=(e-d.y)-d.height/2;return this.transformer(i,["t",(-d.width/2),(d.height/2)])};Raphael.fn.printArcLabel=function(B,A,v,z,e,q,l,k,w,M,d){var g=true;var u=this.set();var E=(g)?((M>0&&M<180)?false:true):true;var G=0,h=0,C,s;var m=new Array();for(var D=0;D<A.length;D++){G=0;C=D;s=this.print(G,h,A.charAt(C),v,z,"middle");m.push(s.getBBox());s.remove();if(A.charAt(C)!="\n"){u.push(this.print(G-m[D].width/2,h-q/2,A.charAt(C),v,z,"middle"))}else{h+=q}}var b=360/(2*Math.PI*w);var L=Math.PI/180;if(w&&M){var H=(q-z)*b;var K=new Array();var N=0;for(var D=0;D<u.length;D++){N=(u[D].getBBox().width+e)*b;H+=N;K.push(N)}H+=(q-z)*b;if(d){var I=[B.makeSector(l,k,M,H/2,w+q),B.makeSector(l,k,M,(H/2+2*b),w+q*1.05)];B.shadows.push(this.path(I[1]).attr({fill:B.colours.deepshadow,stroke:0,cursor:"pointer",opacity:0.8}));B.arc.push(this.path(I[1]).attr({fill:B.colours.frame,stroke:0,cursor:"pointer"}));B.arc.push(this.path(I[0]).attr({fill:B.colours.white,stroke:0,cursor:"pointer"}))}var J=E?M-H/2+(q-z)*b:M+H/2-(q-z)*b;var p,n,o;var F=E;for(var D=0;D<u.length;D++){p=u[D].getBBox().width/2;n=q/2;u[D].toFront();o=w+(E?-(q-z)/2:q);J+=(E?1:-1)*K[D]/2;this.transformer(u[D],["R",(J+90+(E?0:-180)),0,0,"T",(l+o*Math.cos(J*L)),(k+o*Math.sin(J*L))]);J+=(E?1:-1)*K[D]/2}}return u}})();(function($){
/*@cc_on
