/*!
	...
	(c) Stuart Lowe/LCOGT
*/
(function(){var a=function(h,g,f){if(typeof(f)=="string"){f=g.path(f).attr({stroke:"none","text-anchor":"middle"})}for(var c=0;c<h.length;c++){var e=h[c];var d=f.getPointAtLength(e.getBBox().x);var b=e.transform()+"T"+(d.x-e.getBBox().x)+","+(d.y-e.getBBox().y-e.getBBox().height);b+="R"+(d.alpha<360?180+d.alpha:d.alpha);e.transform(b)}};Raphael.fn.printLetters=function(h,e,m,d,l,g){d=d||m/1.5;l=l||m;this.setStart();var b=0,j=0;for(var f=0;f<h.length;f++){if(h.charAt(f)!="\n"){var c=this.print(b,j,h.charAt(f),e,m);b+=d}else{b=0;j+=l}}var k=this.setFinish();if(g){a(k,this,g)}return k}})();(function($){
/*@cc_on
