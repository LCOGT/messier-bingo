...
(function ($) {
	...
	MessierBingo.prototype.drawBox = function(){
		...
		this.dialbg.push(
			this.box.path(makeSector(ox+3,oy,270,52,70)).attr({'fill':this.colours.deepshadow,'stroke':0}).translate('t 1 2'),
			this.box.path(makeSector(ox+3,oy,270,52,70)).attr({'fill':this.colours.framedark,'stroke':0}),
			this.box.path(makeSector(ox+3,oy,270,50,67)).attr({'fill':this.colours.white,'stroke':0}),
			this.box.path(makeSector(ox+3,oy,90,47,68)).attr({'fill':this.colours.deepshadow,'stroke':0}).translate('t 1 2'),
			this.box.path(makeSector(ox+3,oy,90,47,68)).attr({'fill':this.colours.framedark,'stroke':0}),
			this.box.path(makeSector(ox+3,oy,90,44,65)).attr({'fill':this.colours.white,'stroke':0})
		)
		...
		this.frame[0].attr({'fill':this.colours.deepshadow,'opacity':0.8,'stroke':0}).transform('t 1.5 2.5');
		this.frame[1].attr({'fill':this.colours.frame,'stroke':0});

		this.clock = this.box.set();
		ox = 911;
		oy = 223.5;
		this.clock.push(
			this.box.circle(ox,oy,63).attr({'fill':'270-'+this.colours.frameinlay,'stroke':0}),
			this.box.circle(ox,oy,59).attr({'fill':this.colours.shadow,'stroke':0}),
			this.box.circle(ox,oy,57).attr({'fill':'90-'+this.colours.brassdark+'-'+this.colours.brassmed+':34-'+this.colours.brass+':100','stroke':0}),
			this.box.circle(ox,oy,55).attr({'fill':this.colours.white,'stroke':0}),
		...
		this.overlay.push(
			this.box.path(path).attr({'fill':this.colours.shadow,'stroke':0}).transform('t -1 -1.5'),
		...
		this.overlay.push(
			this.box.path(path).attr({'fill':this.colours.shadow,'stroke':0}).transform('t -1 -1'),
			this.box.path(path).attr({'fill':this.colours.white,'stroke':0})
		);
		...
		this.messier.push(
			this.box.image('{{STATIC_URL}}images/messier_eyes.png',130,121,28,6),
			this.box.image('{{STATIC_URL}}images/messier.png',85,80,90,110)
		);

		this.portal = this.box.set();
		this.portal.push(
			this.box.circle(661,455,266).attr({'fill':'270-#d8cda9:0-#cfc4a2:15-#b7ab8f:41-#8f8470:52-#857968:60-#817565:66-#74685b:71-#5f534a:80-'+this.colours.shadow+':82','stroke':0}),
			this.box.circle(661,455,234).attr({'fill':this.colours.deepshadow,'stroke':0}),
			this.box.circle(661,455,231).attr({'fill':'90-#d8cda9:0-#cfc4a2:1-#b7ab8f:5-#8f8470:15-#857968:20-#817565:30-#74685b:51-#5f534a:85-'+this.colours.shadow,'stroke':0}),
			this.box.path('m 661,193 c -144,0 -260.737,116.735 -260.737,260.737 0,144.001 116.737,260.736 260.737,260.736 143.997,0 260.734,-116.735 260.734,-260.736 0,-144.002 -116.737,-260.737 -260.734,-260.737 m 0,490.184 c -126.723,0 -229.449,-102.727 -229.449,-229.447 0,-126.722 102.726,-229.448 229.449,-229.448 126.718,0 229.446,102.726 229.446,229.448 0,126.72 -102.729,229.447 -229.446,229.447').attr({'fill':this.colours.shadow,'stroke':0,'opacity':0.76})
		);
		...
		oy = 100;
		this.dial.push(
			this.box.circle(ox,oy+0.5,26).attr({'fill':this.colours.deepshadow,'stroke':0}),
			this.box.circle(ox,oy,25).attr({'fill':'270-'+this.colours.brass+'-'+this.colours.brassmed+':66-'+this.colours.brassdark,'stroke':0}),
			this.box.circle(ox,oy,20).attr({'fill':this.colours.brass,'stroke':0,'cursor':'pointer'})
		)
		this.dialhandle = this.box.set();
		this.dialhandle.push(
			this.box.path("M "+(ox+0.5)+" "+(oy-6+0.5)+" l 5 10 t -5 30 l 0 0 t -5 -30 z").attr({'fill':this.colours.deepshadow,'stroke':0,'cursor':'pointer'}).transform('R0,'+ox+','+oy),
			this.box.path("M "+ox+" "+(oy-6)+" l 5 10 t -5 27 l 0 0 t -5 -27 z").attr({'fill':'0-'+this.colours.deepshadow+'-'+this.colours.brass+'-'+this.colours.deepshadow,'stroke':0,'cursor':'pointer'}).transform('R0,'+ox+','+oy)
		);
		...
		ox = 331;
		oy = 100;
		this.dialtext = this.box.printLetters('INFORMATION',this.box.getFont("Birch Std"),22,7.2,18,makeCurvePath(ox,oy,270,50,45)).attr({'fill':this.colours.deepshadow,'stroke':0});
		this.dialtexton = this.box.printLetters('ON',this.box.getFont("Birch Std"),22,7.2,18,makeCurvePath(ox,oy,90+this.dialang,8,60)).attr({'fill':this.colours.deepshadow,'stroke':0});
		this.dialtextoff = this.box.printLetters('OFF',this.box.getFont("Birch Std"),22,7.2,18,makeCurvePath(ox,oy,90-this.dialang+4,8,60)).attr({'fill':this.colours.deepshadow,'stroke':0});
		...
		this.nextbutton.push(
			this.box.circle(ox+r*0.01,oy+r*0.02,r).attr({'fill':this.colours.deepshadow,'stroke':0}),
			this.box.circle(ox,oy,r*0.97).attr({'fill':this.colours.frame,'stroke':0}),
			this.box.circle(ox,oy,r*0.80).attr({'fill':'270-'+this.colours.frameinlay,'stroke':0}),
			this.box.circle(ox,oy,r*0.75).attr({'fill':this.colours.white,'stroke':0,'cursor':'pointer'}),
			this.box.path('M'+ox+','+oy+'m'+(-r*0.5)+','+(-r*0.2)+' l'+(r*0.5)+',0 l0,'+(-r*0.2)+' l'+(r*0.5)+','+(r*0.4)+' l'+(-r*0.5)+','+(r*0.4)+' l0,'+(-r*0.2)+'l'+(-r*0.5)+',0 z').attr({'fill':this.colours.shadow,'stroke':0,'cursor':'pointer'})
		);
		this.nextbutton.data('mb',this).click(function(e){ this.data('mb').next(); }).mouseover(function(){ this.data('mb').nextbutton[4].attr({'fill':this.colours.deepshadow}); }).mouseout(function(){ this.data('mb').nextbutton[4].attr({'fill':this.colours.shadow}); });
		...
		this.resetbutton.push(
			...
			this.resetbutton[4] = this.box.path('M'+ox+','+oy+'m '+(-r*0.2)+','+(r*0.4)+' c '+(r*0.6)+',0 '+(r*0.4)+','+(-r*0.4)+' '+(r*0.1)+','+(-r*0.3)+' l 0,'+(r*0.2)+' '+(-r*0.4)+','+(-r*0.4)+' '+(r*0.4)+','+(-r*0.4)+' 0,'+(r*0.2)+' c '+(r*0.1)+',0 '+(r*0.5)+',0 '+(r*0.5)+','+(r*0.4)+' 0,'+(r*0.4)+' '+(-r*0.5)+','+(r*0.4)+' '+(-r*0.5)+','+(r*0.3)+' z').attr({'fill':this.colours.shadow,'stroke':0,'cursor':'pointer','opacity':1})
		);
		this.resetbutton.data('mb',this).click(function(e){ this.data('mb').reset(); }).mouseover(function(){ this.data('mb').resetbutton[4].attr({'fill':this.colours.deepshadow}); }).mouseout(function(){ this.data('mb').resetbutton[4].attr({'fill':this.colours.shadow}); }).hide();
	...
	MessierBingo.prototype.updateInfo = function(i,data){
		$('#sky img').attr('src','{{STATIC_URL}}images/iris.png');
		if(i >= 0){
		...
		if(typeof data==="object"){
			if(data.observation){
				...
				$('#sky img').attr('src','{{STATIC_URL}}images/missing.png');
				$('#panel .credit').html('');
				$('#panel .download').html('');
			}
	...
	function Pantograph(me,ox,oy,w,h,w2,n,p,vert){
		...
		this.group.push(
			this.box.path(this.pathoff).attr({'stroke':this.me.colours.deepshadow,'stroke-width':3}).transform('t 1.5 2.5'),
			this.box.path(this.pathoff).attr({'stroke':this.me.colours.framedark,'stroke-width':4})
		);
